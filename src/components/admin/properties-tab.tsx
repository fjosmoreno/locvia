...
const BADGE_STYLE: Record<string, string> = {
  ...
  RECOMMENDED: "bg-violet-500/15 text-violet-300 border-violet-500/30",
};
        ...
        <div className="p-3 space-y-3">
          {data.properties.map((p) => (
            <PropertyAdminCard
              ...
              onDelete={(pp) => setDeleteTarget(pp)}
...
function PropertyAdminCard({
  ...
  return (
    <div
      className={cn(
        "rounded-xl border bg-card/60 backdrop-blur-sm p-3 space-y-3 transition-all duration-200 hover:bg-card hover:border-primary/30 hover:shadow-md",
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="w-16 h-16 rounded-lg overflow-hidden shrink-0 grid place-items-center text-muted-foreground bg-muted ring-1 ring-border/40 relative">
          ...
          {p.featured && (
            <span className="absolute top-0.5 left-0.5 bg-primary text-primary-foreground text-[9px] font-bold px-1.5 py-0.5 rounded shadow-sm">
              <Star className="w-2.5 h-2.5 inline fill-primary-foreground" />
            </span>
          )}
        </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <h3 className="text-sm font-semibold text-foreground line-clamp-2 leading-snug tracking-tight">
              {p.title}
            </h3>
            <PropertyStatusBadge status={p.status} />
          </div>
          <div className="text-sm font-bold text-primary mt-0.5 price tabular-nums">
          ...
          <div className="text-[11px] text-muted-foreground flex flex-wrap items-center gap-x-3 gap-y-0.5 mt-1">
            ...
            <span className="flex items-center gap-1">
              <CalendarDays className="w-3 h-3" /> {formatDate(p.createdAt)}
            </span>
          </div>
          {(p.agency?.name || p.owner?.user?.name) && (
          ...
          {rejectNote && (
            <div className="mt-2 text-[11px] text-rose-300 bg-rose-500/10 border border-rose-500/30 rounded-md px-2 py-1.5">
              <span className="font-semibold">Nota de rejeição:</span> {rejectNote}
            </div>
          )}
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-border/40">
        ...
        >
          <Star
            className={cn(
              "w-3.5 h-3.5",
              p.featured && "fill-primary text-primary"
            )}
          />
